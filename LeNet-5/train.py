...
def train(model, train_dataset, batch_size, lr):
    global loss
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0) ##shuffle得开，自己数据集是有规律的，bro
...
def val(model, val_dataset, batch_size):
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=True, num_workers=0)  ##shuffle得开，自己数据集是有规律的，bro
    loss_fn = nn.CrossEntropyLoss()
    ...
    correct /= size
    torch.save(model.state_dict(), './Model/LeNet_model' + str(100*correct) + '.pth')
